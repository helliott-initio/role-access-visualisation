...
export function useRoleMap() {
  ...
  const updateMapPrefix = useCallback((mapId: string, newPrefix: string) => {
    ...
    mutate((prev) => {
      ...
      if (!map) return prev;
      const oldPrefix = map.prefix || '';

      const rewriteName = (name: string): string => {
        // Remove old prefix + space if present
        const stripped = oldPrefix && name.startsWith(`${oldPrefix} `)
          ? name.slice(oldPrefix.length + 1)
          : name;
        // Add new prefix + space
        return trimmed ? `${trimmed} ${stripped}` : stripped;
      };
        ...
        maps: prev.maps.map((m) => {
          ...
          return {
            ...
            groups: m.groups.map((g) => ({
              ...g,
              label: rewriteName(g.label),
            })),
            sections: m.sections.map((s) => ({
              ...s,
              name: rewriteName(s.name),
            })),
          };
        }),
      };
